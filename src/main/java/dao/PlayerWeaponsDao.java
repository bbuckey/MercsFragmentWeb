...
public class PlayerWeaponsDao extends BaseDAO{
	...
	public List getAllPlayerWeapons(){
		return super.getSessionFactory().getCurrentSession().createQuery("from PlayerWeaponsEntity").list();
	}
	...
	public void deletePlayerWeapon(Integer mercId){
		PlayerWeaponsEntity merc = (PlayerWeaponsEntity) super.getSessionFactory().getCurrentSession().load(PlayerWeaponsEntity.class, mercId);
		if(merc != null){
	...
	public List getPlayerWeaponsByPlayerID(Integer playerID){
		return super.getSessionFactory().getCurrentSession().createQuery("from PlayerWeaponsEntity pw where pw.player_id = :pid").setParameter("pid", playerID).list();
	}
